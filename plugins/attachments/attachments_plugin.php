...
class AttachmentsPlugin extends JObject
{
	 ...
	 * @param string $default_entity Name of default entity for the parent_type
	 *
	 */
	function __construct($extension_name, $parent_type, $default_entity)
	{
		parent::__construct();

		$this->_parent_type = $parent_type;
		$this->_name = $extension_name;
		$this->_default_entity = $default_entity;
		$this->_entities[] = $default_entity;
		$this->_entity_name[$default_entity] = $default_entity;

		// Set up the default alias
		$this->_entity_alias = Array( 'default' => $default_entity );
	...
	/**
	 * Get the proper extension entity name
	 * (eg, 'article' instead of 'default', take care of aliases)
	 *
	...
	function parentExists($parent_id, $parent_entity='default')
	{
		...
		if ( $result === null ) {
			...
			return (int)$parent_id == (int)$result;
			}
	}
