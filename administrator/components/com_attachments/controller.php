...
class AttachmentsAdminController extends JController
{
	...
	function listAttachments()
	{
		...
		$view->version = AttachmentsExtensionVersion;
		$view->project_url = AttachmentsProjectURL;
		...
		if ( $change_parent ) {
			...
			if ( $new_parent_type ) {
				...
				if ( $new_parent_entity == 'default' ) {
					$new_parent_entity = $parent->getDefaultEntity();
					}
	...
	function save()
	{
		...
		if ( $row->parent_type == 'com_content' AND
			 ($row->parent_entity == 'default' OR
			  $row->parent_entity == 'article' OR
			  $row->parent_entity == 'ARTICLE' ) AND
			 $row->parent_id == 0 ) {
			$row->parent_id = null;
			}
