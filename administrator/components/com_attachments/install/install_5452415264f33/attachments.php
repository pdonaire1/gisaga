 ...
 * @return a list of attachments as HTML code
 */
function attachments_attachmentListHTML($parent_id, $parent_type, $parent_entity, $parent,
 ...
 * @return the HTML for the "Add Attachments" link
 */
function attachments_attachmentButtonsHTML($parent_id, $parent_type, $parent_entity, $parent,
										   $Itemid, $from)
{
	global $option;
	...
	if ( $option == 'com_content' AND $parent_entity == 'default' ) {
		...
		if ( $parent_entity != $parent->getDefaultEntity() ) {
	...
	$ahead .= "rel=\"{handler: 'iframe', size: {x: 800, y: 550}}\">";
...
function addAttachments( &$row, &$params, $page=0 )
{
	...
	if ( ( $who_can_see == 'anyone' ) OR
		...
		$attachments_list =
			attachments_attachmentListHTML($parent_id, $parent_type, $parent_entity, $parent, $user_can_add, $Itemid, $from);
	...
	if ( $user_can_add ) {
		$add_attachments_btn =
			attachments_attachmentButtonsHTML($parent_id, $parent_type, $parent_entity, $parent, $Itemid, $from);
