...
class AttachmentsUpdate
{
	...
	function update_attachments_table()
	{
		...
		$new_fields['display_name'] =
			"ALTER TABLE #__attachments ADD COLUMN `display_name` VARCHAR(80) NOT NULL DEFAULT ''";
		$new_fields['uri_type'] =
			"ALTER TABLE #__attachments ADD COLUMN `uri_type` ENUM('file', 'url') DEFAULT 'file'";
		$new_fields['url_valid'] =
			"ALTER TABLE #__attachments ADD COLUMN `url_valid` TINYINT(1) UNSIGNED NOT NULL DEFAULT '0'";
		$new_fields['user_field_1'] =
			"ALTER TABLE #__attachments ADD COLUMN `user_field_1` VARCHAR(100) NOT NULL DEFAULT ''";
		$new_fields['user_field_2'] =
			"ALTER TABLE #__attachments ADD COLUMN `user_field_2` VARCHAR(100) NOT NULL DEFAULT ''";
		$new_fields['user_field_3'] =
			"ALTER TABLE #__attachments ADD COLUMN `user_field_3` VARCHAR(100) NOT NULL DEFAULT ''";
		$new_fields['create_date'] =
			"ALTER TABLE #__attachments ADD COLUMN `create_date` DATETIME DEFAULT NULL";
		$new_fields['modification_date'] =
			"ALTER TABLE #__attachments ADD COLUMN `modification_date` DATETIME DEFAULT NULL";
		$new_fields['download_count'] =
			"ALTER TABLE #__attachments ADD COLUMN `download_count` INT(11) UNSIGNED DEFAULT '0'";
		$new_fields['parent_type'] =
			"ALTER TABLE #__attachments ADD COLUMN `parent_type` VARCHAR(100) NOT NULL DEFAULT 'com_content'";
		$new_fields['parent_entity'] =
			"ALTER TABLE #__attachments ADD COLUMN `parent_entity` VARCHAR(100) NOT NULL DEFAULT 'default'";
		...
		// Change the case of any existing parent_entities to lower case
		$query = "UPDATE #__attachments SET `parent_entity` = LOWER(`parent_entity`)";
		$db->setQuery($query);
		if (!$db->query()) {
			$errmsg = JText::sprintf('ERROR_UPDATING_EXISTING_ATTACHMENTS')	 .
				' ' . $db->stderr() . ' (ERRN)';
