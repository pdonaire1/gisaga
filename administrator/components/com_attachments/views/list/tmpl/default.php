 ...
 <thead>
   <tr>
	...
	<input type="checkbox" name="toggle" value=""
	 ...
	 <th class="at_published" width="5%" nowrap="nowrap"><?php echo JHTML::_('grid.sort', 'PUBLISHED',
							 'a.published', @$lists['order_Dir'], @$lists['order'] ) ?></th>
	 <th class="at_filename"><?php echo JHTML::_('grid.sort', 'ATTACHMENT_FILENAME',
							 'a.filename', @$lists['order_Dir'], @$lists['order'] ) ?></th>
	 <th class="at_description"><?php echo JHTML::_('grid.sort', 'DESCRIPTION',
	...
	<th class="at_user_field"><?php echo JHTML::_('grid.sort', $params->get('user_field_2_name', ''),
	 ...
	 <?php endif; ?>
	 <th class="at_file_type"><?php echo JHTML::_('grid.sort', 'FILE_TYPE',
							 'a.file_type', @$lists['order_Dir'], @$lists['order'] ) ?></th>
	 <th class="at_file_size"><?php echo JHTML::_('grid.sort', 'FILE_SIZE_KB',
							 'a.file_size', @$lists['order_Dir'], @$lists['order'] ) ?></th>
	 <th class="at_uploader"><?php echo JHTML::_('grid.sort', 'UPLOADER',
							 'u.name', @$lists['order_Dir'], @$lists['order'] ) ?></th>
	 <th class="at_create_date"><?php echo JHTML::_('grid.sort', 'CREATED',
								'a.create_date', @$lists['order_Dir'], @$lists['order'] ) ?></th>
	 <th class="at_mod_date"><?php echo JHTML::_('grid.sort', 'LAST_MODIFIED',
							 'a.modification_date', @$lists['order_Dir'], @$lists['order'] ) ?></th>
	 <?php if ( $secure ):	$num_columns++; ?>
	 <th class="at_downloads"><?php echo JHTML::_('grid.sort', 'DOWNLOADS',
...
for ($i=0, $n=count( $this->attachments ); $i < $n; $i++) {
	$row =& $this->attachments[$i];
	 ...
	 $row_parent_entity = $row->parent_entity;
	 if ($row->parent_id != $last_parent_id OR $row->parent_type != $last_parent_type
		 OR $row_parent_entity != $last_parent_entity ) {
		 $parent_type = $row->parent_type;
		 if ( $row_parent_entity != 'default' ) {
			 $parent_type .= '.' . $row_parent_entity;
			 }
		 if ( $row->parent_id == null OR !$row->parent_exists ) {
			 ...
			 $artLine .= "<b>".$row->parent_entity_type.":</b> <a title=\"$view_parent_title\" " .
				 "href=\"".$row->parent_url."\" target=\"_blank\">" . $row->parent_title . "</a>";
