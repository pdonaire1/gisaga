...
define('AttachmentsExtensionVersion', '2.2.2');
define('AttachmentsProjectURL', 'http://joomlacode.org/gf/project/attachments/');
...
class AttachmentsHelper
{
	...
	function addStyleSheet($css_path, $echo=false)
	{
		...
		elseif ( $caching ) {
			...
			if ( !$added_js ) {
				$js_path = JURI::base(true) . '/plugins/content/attachments_caching.js';
	 ...
	 */
	function upload_file(&$attachment, &$parent, $attachment_id=false, $save_type='update')
	{
		global $mainframe;
		$db =& JFactory::getDBO();
		...
		if ( (($_FILES['upload']['size'] == 0) &&
			...
			if ( $bad_filename ) {
				...
				$error_msg = JText::_('ERROR_ILLEGAL_FILE_EXTENSION') . " .php.$format";
				if ( $mainframe->isAdmin() ) {
					$result = new JObject();
					$result->error = true;
			...
			if ( $save_type == 'update' ) {
				...
				$view->assignRef('uri_type',		 $attachment->uri_type);
				$view->assignRef('url',				 $attachment->url);
				$view->assign(	 'parent_id',		 $attachment->parent_id);
				$view->assignRef('parent_type',		 $attachment->parent_type);
				$view->assignRef('description',		 $attachment->description);
				$view->assignRef('user_field_1',	 $attachment->user_field_1);
				$view->assignRef('user_field_2',	 $attachment->user_field_2);
				$view->assignRef('user_field_3',	 $attachment->user_field_3);
				}
			...
			// Set up the view
			$view->assignRef('parent_entity',	 $attachment->parent_entity);
		...
		if (AttachmentsHelper::is_image_file($filename)) {
			if (!AttachmentsHelper::is_valid_image_file($_FILES['upload']['tmp_name'])) {
				$error = 'illegal_file_extension';
				$error_msg = JText::sprintf('ERROR_UPLOADING_FILE_S', $filename);
				$error_msg .= "<br />" . JText::_('ERROR_ILLEGAL_FILE_EXTENSION') . " (corrupted image file)";
				}
			}
		...
		if ( $duplicate_filename ) {
			...
			$view->assign(	 'new_parent',		 $parent->new);
			$view->assignRef('description',		 $attachment->description);
			$view->assignRef('display_name',	 $attachment->display_name);
			$view->assignRef('user_field_1',	 $attachment->user_field_1);
			$view->assignRef('user_field_2',	 $attachment->user_field_2);
			$view->assignRef('user_field_3',	 $attachment->user_field_3);
		...
		$now = new JDate();
		$attachment->create_date = $now->toMySQL();
		$attachment->modification_date = $attachment->create_date;
	...
	function download_attachment($id)
	{
		...
		// Make sure the user can access the parent article (only if it is an article)
		if ( $parent_entity == 'com_content' AND
			...
			$amodel = new ContentModelArticle();
			$amodel->setId( $attachment->parent_id );
			$article = $amodel->getArticle();
			// NOTE: If the user cannot access the article because of section/category
			//       restrictions, getArticle() will raise a 403/404 error

			// Abort if no matching article is found
			if ( $article->text == '' ) {
				$errmsg = JText::_('ERROR_UNKNOWN_PARENT_ID') . ' (ERR)';
				JError::raiseError(500, $errmsg);
				}

			// Abort if the user does not have permission to see the parent article
			if ( $article->access > $user->get('aid', 0) ) {
				$errmsg = JText::_('ERROR_NO_PERMISSION_TO_DOWNLOAD') . ' (ERR)';
				JError::raiseError(500, $errmsg);
				}
			}
